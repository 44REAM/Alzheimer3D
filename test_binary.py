...
def run_test(test_loader, model, device):
    ...
    with torch.no_grad():
        for test_images, test_labels in tqdm(test_loader):
            ...
            test_outputs = test_outputs.sigmoid().cpu()
            test_labels = test_labels.cpu()

            test_outputs_all.append(test_outputs)
            test_labels_all.append(test_labels)

    test_outputs_all = torch.cat(test_outputs_all)
...
def change_label(X_train, X_val, X_test, y_train, y_val,y_test):
    ...
    y_train = np.array([label for name, label in zip(X_train, y_train) if label != 1])
    y_val = np.array([label for name, label in zip(X_val, y_val) if label != 1])
    y_test = np.array([label for name, label in zip(X_test, y_test) if label != 1])
    ...
    for mo, norm in zip(modalities, should_normalize):
        ...
        for model_name in all_model:
            ...
            if change_label_or_not:
                X_train, X_val, X_test, y_train, y_val, y_test = change_label(X_train, X_val, X_test, y_train, y_val, y_test)
            ...
            try:
                ...
                model = get_model(model_name, n_classes=1)
                model = model.to(device)
                model.load_state_dict(load_checkpoint(
                    checkpoints_dir, checkpoint_name, 'bacc'))

                pos_weight = torch.tensor((len(y_train) - np.sum(y_train))/np.sum(y_train))
